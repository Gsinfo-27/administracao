...
import admin.gestao.services.UserService;
...
public class UsersController {
    ...
    public ResponseEntity<LoginResponse> login(@RequestBody Login loginRequest) {
        ...
        if (authentication.isAuthenticated()) {
            var token = generateToken.generateToken(userService.findByName(loginRequest.getUserName()), LocalDateTime.now().plusMinutes(1).toInstant(ZoneOffset.of("+02:00")));
